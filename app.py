...
def run_ep_scan(min_gap_pct, min_price, asset_filter, exchange_filter,
    ...
    total, done, results = len(candidates), 0, []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(phase2_ep_confirm, c, min_vol, min_price): c for c in candidates}
        for fut in as_completed(futs):
            done += 1
...
def phase2_breakout_confirm(candidate: dict, min_vol: int, min_price: float = 0.0) -> dict | None:
...
def run_breakout_scan(min_perf_1m, min_price, asset_filter, exchange_filter,
    ...
    total, done, results = len(candidates), 0, []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(phase2_breakout_confirm, c, min_vol, min_price): c for c in candidates}
        for fut in as_completed(futs):
            done += 1
            p2_cb(done / total, done, total)
            res = fut.result()
...
def phase2_pl_confirm(candidate: dict, min_vol: int, min_price: float = 0.0) -> dict | None:
...
def run_parabolic_long_scan(min_drop_5d, min_price, asset_filter, exchange_filter,
    ...
    total, done, results = len(candidates), 0, []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(phase2_pl_confirm, c, min_vol, min_price): c for c in candidates}
        for fut in as_completed(futs):
            done += 1
            p2_cb(done / total, done, total)
            res = fut.result()
...
# Jeff Sun's documented 38-name Liquid Mega List (updated Dec 2025, 1M RS sorted)
MEGA_LIST_DEFAULT = [
    "NVDA", "META", "MSFT", "AAPL", "AMZN", "GOOGL", "TSLA", "AMD",
    "AVGO", "APP",  "PLTR", "NFLX", "UNH",  "MU",   "MRVL", "ORCL",
    "COIN", "HOOD", "MSTR", "UPST", "RDDT", "SOFI",  "SMCI", "HIMS",
    "BABA", "INTC", "RIOT", "RGTI", "QBTS", "QUBT",  "IONQ", "OKLO",
    "SOUN", "SMR",  "TEM",  "CRWV", "CRCL", "IONQ",
]
MEGA_LIST_DEFAULT = list(dict.fromkeys(MEGA_LIST_DEFAULT))  # dedupe

# Jeff's 5 focus sectors (64% of SPY weighting — starred in his diary)
FOCUS_SECTORS = {"XLK", "XLV", "XLF", "XLY", "XLC"}
...
INDEX_META = {
    "SPY":  ("S&P 500",      "cap-wt"),
    "QQQ":  ("Nasdaq 100",   "cap-wt"),
    "RSP":  ("S&P 500 EW",   "equal-wt"),
    "QQQE": ("QQQ EW",       "equal-wt"),
...
def fetch_breadth_from_tv() -> dict:
    """
    Use TradingView screener to compute market breadth proxies:
    - % of US stocks above their 20-MA  (MMTW proxy)
    - % of US stocks above their 50-MA
    - Stocks at/near 52-week highs vs lows
    """
    result = {"mmtw_proxy": None, "mm50_proxy": None,
              "pct_near_52h": None, "pct_near_52l": None,
              "total_scanned": None, "error": None}
    try:
        from tradingview_screener import Query, Column
        q_total = (
            Query().set_markets("america")
            .select("name", "close", "SMA20", "SMA50",
                    "price_52_week_high", "price_52_week_low")   # correct column names
            .where(
                Column("type") == "stock",
        ...
        _, df = q_total.get_scanner_data()
        if df is None or df.empty:
            result["error"] = "No data from TradingView"
            return result

        # Bug #1 fix: don't fillna(0) for MA columns — that inflates above-MA counts.
        # Instead, only count rows where the MA is actually available (> 0).
        total = len(df)
        result["total_scanned"] = total

        valid20 = df[df["SMA20"].notna() & (df["SMA20"] > 0)]
        valid50 = df[df["SMA50"].notna() & (df["SMA50"] > 0)]
        above20  = (valid20["close"] > valid20["SMA20"]).sum()
        above50  = (valid50["close"] > valid50["SMA50"]).sum()

        # 52w NH/NL: use correct column names, fill NaN with 0 only for these
        h52 = df["price_52_week_high"].fillna(0)
        l52 = df["price_52_week_low"].fillna(0)
        near_52h = ((df["close"] >= h52 * 0.97) & (h52 > 0)).sum()
        near_52l = ((df["close"] <= l52 * 1.03) & (l52 > 0)).sum()

        n20 = len(valid20) or 1
        n50 = len(valid50) or 1
        result["mmtw_proxy"]   = round(above20 / n20 * 100, 1)
        result["mm50_proxy"]   = round(above50 / n50 * 100, 1)
        result["pct_near_52h"] = round(near_52h / total * 100, 1) if total else None
        result["pct_near_52l"] = round(near_52l / total * 100, 1) if total else None
        result["nh_count"]     = int(near_52h)
        result["nl_count"]     = int(near_52l)
        result["nh_nl_net"]    = int(near_52h) - int(near_52l)
    except Exception as e:
        result["error"] = str(e)
    return result
...
def _color(val: float, good_above=0.0, warn_above=None, reverse=False) -> str:
    """Return CSS color class for a value.

    Normal (reverse=False): higher is better.
      val >= good_above → pos,  val >= warn_above → warn,  else neg
    Reverse (reverse=True): used for pct_52h (less negative = closer to highs = better).
      val >= good_above → pos,  val >= warn_above → warn,  else neg
      e.g. _color(-2, -5, -15, reverse=True):  -2 >= -5 → pos  ✓
           _color(-8, -5, -15, reverse=True):  -8 >= -15 → warn ✓
           _color(-25,-5, -15, reverse=True):  -25 < -15 → neg  ✓
    """
    if reverse:
        if val >= good_above:                          return "pos"
        if warn_above is not None and val >= warn_above: return "warn"
        return "neg"
    else:
        if val >= good_above:                          return "pos"
        if warn_above is not None and val >= warn_above: return "warn"
...
tab_dash, tab_ps, tab_ep, tab_bo, tab_pl, tab_guide = st.tabs([
    "📊  Market Dashboard",
    "📉  Parabolic Short",
    "⚡  Episodic Pivot",
    "🚀  Momentum Breakout",
...
.dash-section-title {
  font-family: 'Geist Mono', monospace; font-size: 0.6rem; font-weight: 600;
...
.mc-label { font-family: 'Geist Mono', monospace; font-size: 0.55rem;
            color: var(--dim); text-transform: uppercase; letter-spacing: 0.1em; }
.mc-val   { font-family: 'Geist Mono', monospace; font-size: 1.1rem;
            font-weight: 700; color: var(--bright); }
.mc-sub   { font-family: 'Geist Mono', monospace; font-size: 0.62rem; color: var(--muted); }
.mc-val.pos { color: var(--green); }
.mc-val.neg { color: var(--red); }
.mc-val.warn { color: var(--amber); }

/* Breadth gauge */
.breadth-bar-wrap { background: var(--bg3); border-radius: 4px; height: 8px;
                    overflow: hidden; margin: 0.3rem 0; }
.breadth-bar { height: 100%; border-radius: 4px; transition: width 0.4s; }

/* Index / sector table rows */
.dash-row {
  display: grid; gap: 0; border-bottom: 1px solid var(--line);
  padding: 0.38rem 0.5rem; align-items: center;
}
.dash-row:last-child { border-bottom: none; }
.dash-row:hover { background: var(--bg3); }
.dash-tbl { background: var(--bg2); border: 1px solid var(--line);
            border-radius: 6px; overflow: hidden; }
.dash-th {
  font-family: 'Geist Mono', monospace; font-size: 0.52rem; font-weight: 600;
  letter-spacing: 0.1em; text-transform: uppercase; color: var(--dim);
  background: var(--bg1); border-bottom: 1px solid var(--line);
  padding: 0.4rem 0.5rem; display: grid; gap: 0;
}
.dash-cell { font-family: 'Geist Mono', monospace; font-size: 0.72rem;
             color: var(--text); }
.dash-cell.name { font-weight: 600; color: var(--bright); }
.dash-cell.sub  { font-size: 0.57rem; color: var(--dim); }
.dash-cell.pos  { color: var(--green); }
.dash-cell.neg  { color: var(--red); }
.dash-cell.warn { color: var(--amber); }
.dash-cell.star { color: var(--amber); }

/* Mini bar for 52w range */
.range-bar-wrap { background: var(--bg3); border-radius: 3px; height: 5px;
                  width: 60px; display: inline-block; vertical-align: middle; overflow: hidden; }
.range-bar { height: 100%; border-radius: 3px; background: var(--green); }
.range-bar.warn { background: var(--amber); }
.range-bar.neg  { background: var(--red); }
...
.style-val   { font-family: 'Geist Mono', monospace; font-size: 0.9rem;
               font-weight: 700; margin: 0.15rem 0; }
.style-sub   { font-family: 'Geist Mono', monospace; font-size: 0.58rem; color: var(--muted); }

/* Mega list table */
.mega-row { display: grid; gap: 0; padding: 0.35rem 0.5rem;
            border-bottom: 1px solid var(--line); align-items: center; }
.mega-row:last-child { border-bottom: none; }
.mega-row:hover { background: var(--bg3); }
.rank-badge {
  ...
  padding: 0.08rem 0.35rem; color: var(--muted); display: inline-block;
}
.rank-badge.top5  { background: rgba(52,211,153,.15); color: var(--green); border-color: rgba(52,211,153,.3); }
.rank-badge.top15 { background: rgba(245,158,11,.1);  color: var(--amber); border-color: rgba(245,158,11,.2); }

/* Bias banner */
.bias-bull  { background: rgba(52,211,153,.08); border: 1px solid rgba(52,211,153,.25);
              border-radius: 8px; padding: 0.8rem 1.2rem; }
.bias-bear  { background: rgba(248,113,113,.08); border: 1px solid rgba(248,113,113,.25);
              border-radius: 8px; padding: 0.8rem 1.2rem; }
.bias-neut  { background: rgba(245,158,11,.08);  border: 1px solid rgba(245,158,11,.2);
              border-radius: 8px; padding: 0.8rem 1.2rem; }
.bias-title { font-family: 'Geist Mono', monospace; font-size: 0.85rem; font-weight: 700;
              letter-spacing: 0.04em; }
.bias-detail { font-family: 'DM Sans', sans-serif; font-size: 0.78rem;
...
""", unsafe_allow_html=True)
    ...
    with col_hdr:
        st.markdown("""
<p style="font-family:'Geist Mono',monospace;font-size:0.6rem;color:var(--dim);
letter-spacing:0.18em;text-transform:uppercase;margin:0">
Jeff Sun / jfsrev — Daily Market Dashboard · Breadth · Indices · Sectors · Mega List</p>""",
        unsafe_allow_html=True)
    with col_btn:
    ...
    if "dash_loaded" not in st.session_state or refresh:
    ...
    if not st.session_state["dash_loaded"]:
        with st.spinner("Fetching market data…"):
            ...
            st.session_state["dash_ts"]      = datetime.now().strftime("%H:%M:%S")
    else:
        mdata   = st.session_state.get("dash_mdata", {})
        breadth = st.session_state.get("dash_breadth", {})

    ts = st.session_state.get("dash_ts", "–")
    st.markdown(f"<p style='font-family:Geist Mono,monospace;font-size:0.55rem;"
                f"color:var(--dim);margin:0.2rem 0 0.8rem'>Last updated: {ts} "
                f"· Data: yfinance + TradingView screener · Cached 5 min</p>",
                unsafe_allow_html=True)

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 1 — BREADTH (MMTW proxy + NH/NL)
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">① Market Breadth</div>', unsafe_allow_html=True)

    mmtw  = breadth.get("mmtw_proxy")
    mm50  = breadth.get("mm50_proxy")
    nh    = breadth.get("nh_count", 0) or 0
    nl    = breadth.get("nl_count", 0) or 0
    nhnl  = breadth.get("nh_nl_net", 0) or 0
    tot   = breadth.get("total_scanned", 0) or 0

    def mmtw_bias(v):
        if v is None: return "warn", "–"
        if v >= 65: return "pos", "STRONG BULL"
        if v >= 50: return "pos", "BULLISH"
        if v >= 40: return "warn", "NEUTRAL"
        if v >= 30: return "warn", "WEAK"
        return "neg", "BEARISH"
    ...
    mmtw_cls, mmtw_lbl = mmtw_bias(mmtw)
    mm50_cls, mm50_lbl = mmtw_bias(mm50)
    nhnl_cls = nhnl_bias(nhnl)

    # Pre-compute display strings to avoid malformed f-string ternary expressions (Bug #3 fix)
    mmtw_disp  = f"{mmtw:.1f}%" if mmtw is not None else "–"
    mm50_disp  = f"{mm50:.1f}%" if mm50 is not None else "–"
    bar_w      = f"{mmtw:.0f}" if mmtw is not None else "0"
    mm50_bar_w = f"{mm50:.0f}" if mm50 is not None else "0"
    bar_cls    = "pos" if mmtw and mmtw >= 50 else ("warn" if mmtw and mmtw >= 35 else "neg")
    bar_col    = "#34d399" if bar_cls == "pos" else ("#f59e0b" if bar_cls == "warn" else "#f87171")
    bar_cls50  = "pos" if mm50 and mm50 >= 50 else ("warn" if mm50 and mm50 >= 35 else "neg")
    bar_col50  = "#34d399" if bar_cls50 == "pos" else ("#f59e0b" if bar_cls50 == "warn" else "#f87171")

    bc1, bc2, bc3, bc4 = st.columns(4)
    bc1.markdown(f"""
<div class="metric-cell">
  <span class="mc-label">% Above 20-MA <span style="color:var(--amber)">(MMTW proxy)</span></span>
  <span class="mc-val {mmtw_cls}">{mmtw_disp}</span>
  <div class="breadth-bar-wrap"><div class="breadth-bar" style="width:{bar_w}%;background:{bar_col}"></div></div>
  <span class="mc-sub">{mmtw_lbl} · Jeff threshold: >50% = bullish</span>
</div>""", unsafe_allow_html=True)

    bc2.markdown(f"""
<div class="metric-cell">
  <span class="mc-label">% Above 50-MA</span>
  <span class="mc-val {mm50_cls}">{mm50_disp}</span>
  <div class="breadth-bar-wrap"><div class="breadth-bar" style="width:{mm50_bar_w}%;background:{bar_col50}"></div></div>
  <span class="mc-sub">{mm50_lbl}</span>
</div>""", unsafe_allow_html=True)

    bc3.markdown(f"""
<div class="metric-cell">
  <span class="mc-label">NH / NL (near 52w) <span style="color:var(--dim)">±3%</span></span>
  <span class="mc-val {nhnl_cls}">{nhnl:+d}</span>
  <span class="mc-sub">New highs: {nh} · New lows: {nl}</span>
</div>""", unsafe_allow_html=True)

    # Overall market bias synthesis
    score = 0
    if mmtw:
        if mmtw >= 60: score += 2
        elif mmtw >= 50: score += 1
        elif mmtw < 40: score -= 1
    if nhnl >= 100: score += 1
    elif nhnl < -100: score -= 1

    if score >= 2:   bias_cls, bias_icon, bias_txt = "bias-bull", "🟢", "RISK-ON — Be aggressive with setups"
    elif score >= 1: bias_cls, bias_icon, bias_txt = "bias-bull", "🟡", "LEANING BULL — Selective, higher bar"
    elif score <= -2: bias_cls, bias_icon, bias_txt = "bias-bear", "🔴", "RISK-OFF — Sit on hands, reduce exposure"
    else:             bias_cls, bias_icon, bias_txt = "bias-neut", "🟡", "NEUTRAL — Wait for cleaner conditions"

    bc4.markdown(f"""
<div class="{bias_cls}">
  <div class="bias-title">{bias_icon} {bias_txt}</div>
  <div class="bias-detail">Scanned {tot:,} US stocks · Jeff: "The crucial thing is to avoid playing when you don't see a fat pitch"</div>
</div>""", unsafe_allow_html=True)

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 2 — MARKET INDEX LEVEL
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">② Market Level — Equal-Weight vs Cap-Weight (RSP vs SPY, QQQE vs QQQ)</div>',
                unsafe_allow_html=True)

    cols_idx = "2fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr"
    st.markdown(f"""
<div class="dash-tbl">
  <div class="dash-th" style="grid-template-columns:{cols_idx}">
    <span>Index</span><span>Price</span><span>Day %</span>
    <span>1W %</span><span>1M %</span><span>3M %</span>
    <span>vs 52H</span><span>vs 20MA</span>
  </div>""", unsafe_allow_html=True)

    for sym, (name, note) in INDEX_META.items():
        d = mdata.get(sym)
        if not d:
            st.markdown(f'<div class="dash-row" style="grid-template-columns:{cols_idx}">'
                        f'<span class="dash-cell name">{sym}</span>'
                        f'<span class="dash-cell sub" style="grid-column:2/9">loading…</span></div>',
                        unsafe_allow_html=True)
            continue
        dc  = _color(d["day_chg"])
        wc  = _color(d["perf_1w"])
        mc  = _color(d["perf_1m"])
        qc  = _color(d["perf_3m"])
        hc  = _color(d["pct_52h"], reverse=True)   # closer to 0 = near highs = good
        ma_cls = "pos" if d["above_20"] else "neg"
        ma_txt = f'+{d["sma20"] and round((d["last"]/d["sma20"]-1)*100,1)}%' if d["above_20"] and d["sma20"] else (
                 f'{d["sma20"] and round((d["last"]/d["sma20"]-1)*100,1)}%' if d["sma20"] else "–")
        note_span = f'<span class="dash-cell sub"> {note}</span>' if note else ""
        st.markdown(f"""
<div class="dash-row" style="grid-template-columns:{cols_idx}">
  <span class="dash-cell name">{sym} <span class="dash-cell sub">{name}</span></span>
  <span class="dash-cell">${d['last']:,.2f}</span>
  <span class="dash-cell {dc}">{d['day_chg']:+.2f}%</span>
  <span class="dash-cell {wc}">{d['perf_1w']:+.1f}%</span>
  <span class="dash-cell {mc}">{d['perf_1m']:+.1f}%</span>
  <span class="dash-cell {qc}">{d['perf_3m']:+.1f}%</span>
  <span class="dash-cell {hc}">{d['pct_52h']:+.1f}%</span>
  <span class="dash-cell {ma_cls}">{'▲' if d['above_20'] else '▼'} {ma_txt}</span>
</div>""", unsafe_allow_html=True)

    st.markdown('</div>', unsafe_allow_html=True)

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 3 — SUB-MARKET (iShares size × style matrix)
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">③ Sub-Market — iShares Size × Style Matrix (Jeff: outperformance/underperformance by daily %)</div>',
                unsafe_allow_html=True)

    style_order = ["IVW", "IVE", "IJK", "IJJ", "IJT", "IJS"]
    sm_cells = ""
    for sym in style_order:
        d   = mdata.get(sym)
        lbl, code = STYLE_META.get(sym, (sym, sym))
        if not d:
            sm_cells += f'<div class="style-cell"><div class="style-label">{code}</div><div class="style-val warn">–</div><div class="style-sub">{lbl}</div></div>'
            continue
        dc  = _color(d["day_chg"])
        mc  = _color(d["perf_1m"])
...
<div class="style-cell">
  ...
  <div class="style-sub">1M <span class="{mc}">{d['perf_1m']:+.1f}%</span> · 52H <span class="{hc}">{d['pct_52h']:+.1f}%</span></div>
</div>"""

    st.markdown(f'<div class="style-grid">{sm_cells}</div>', unsafe_allow_html=True)

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 4 — SECTOR LEVEL
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">④ Sector Level — 11 SPDR Sectors (⭐ = Jeff\'s 5 focus sectors, 64% of SPY weighting)</div>',
                unsafe_allow_html=True)

    # Sort sectors by 1M performance (Jeff sorts by % gain from 52w low)
    sector_rows = []
    for sym, (name, star) in SECTOR_META.items():
        d = mdata.get(sym)
        if d:
            sector_rows.append((sym, name, star, d))
    sector_rows.sort(key=lambda x: x[3]["pct_52l"], reverse=True)

    sec_cols = "1.2fr 1.5fr 1fr 1fr 1fr 1fr 1fr 3fr"
    st.markdown(f"""
<div class="dash-tbl">
  <div class="dash-th" style="grid-template-columns:{sec_cols}">
    <span>ETF</span><span>Sector</span><span>Day %</span>
    <span>1W %</span><span>1M %</span><span>vs 52H</span><span>vs 52L</span>
    <span>52-Week Range Position</span>
  </div>""", unsafe_allow_html=True)

    for sym, name, star, d in sector_rows:
        dc = _color(d["day_chg"])
        wc = _color(d["perf_1w"])
        mc = _color(d["perf_1m"])
        hc = _color(d["pct_52h"], -5, -15, reverse=True)
        ...
        st.markdown(f"""
<div class="dash-row" style="grid-template-columns:{sec_cols}">
  <span class="dash-cell name">{sym}{star_span}</span>
  <span class="dash-cell">{name}</span>
  <span class="dash-cell {dc}">{d['day_chg']:+.2f}%</span>
  <span class="dash-cell {wc}">{d['perf_1w']:+.1f}%</span>
  <span class="dash-cell {mc}">{d['perf_1m']:+.1f}%</span>
  <span class="dash-cell {hc}">{d['pct_52h']:+.1f}%</span>
  <span class="dash-cell {lc}">+{d['pct_52l']:.1f}%</span>
  <span class="dash-cell">
    <div class="range-bar-wrap" style="width:100%"><div class="range-bar" style="width:{pos:.0f}%;background:{bar_c}"></div></div>
  </span>
</div>""", unsafe_allow_html=True)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 5 — LIQUID MEGA LIST
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">⑤ Liquid Mega List — Jeff Sun\'s 38-name focus universe, sorted by 1M RS</div>',
                unsafe_allow_html=True)

    # User can edit the list
    with st.expander("✏  Edit Mega List", expanded=False):
        custom = st.text_area("Tickers (comma-separated)",
                              value=", ".join(MEGA_LIST_DEFAULT),
                              height=80, key="mega_list_input")
    ...
    mega_tickers = [t.strip().upper() for t in
                    st.session_state.get("mega_list_input", ", ".join(MEGA_LIST_DEFAULT)).split(",")
                    if t.strip()]
    if not mega_tickers:
        mega_tickers = MEGA_LIST_DEFAULT

    # Build and sort by 1M performance (RS rank)
    mega_rows = []
    for t in mega_tickers:
        d = mdata.get(t)
        if d:
            mega_rows.append(d)
    mega_rows.sort(key=lambda x: x["perf_1m"], reverse=True)

    mega_cols = "0.4fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr"
    st.markdown(f"""
<div class="dash-tbl">
  <div class="dash-th" style="grid-template-columns:{mega_cols}">
    <span>RS</span><span>Ticker</span><span>Price</span>
    <span>Day %</span><span>1W %</span><span>1M %</span><span>3M %</span>
    <span>ADR%</span><span>vs 52H</span>
  </div>""", unsafe_allow_html=True)

    for rank, d in enumerate(mega_rows, 1):
        rank_cls  = "top5" if rank <= 5 else ("top15" if rank <= 15 else "")
        dc  = _color(d["day_chg"])
        wc  = _color(d["perf_1w"])
        mc  = _color(d["perf_1m"])
        qc  = _color(d["perf_3m"])
        hc  = _color(d["pct_52h"], -5, -20, reverse=True)
        adr = d.get("adr_pct")
        adr_cls  = "pos" if adr and adr >= 5 else ("warn" if adr and adr >= 3 else "neg")
        adr_disp = f"{adr:.1f}%" if adr else "–"
        ma_dot   = "🟢" if d["above_20"] else "🔴"
        st.markdown(f"""
<div class="mega-row" style="grid-template-columns:{mega_cols}">
  <span class="rank-badge {rank_cls}">#{rank}</span>
  <span class="dash-cell name">{d['ticker']} <span style="font-size:0.55rem">{ma_dot}</span></span>
  <span class="dash-cell">${d['last']:,.2f}</span>
  <span class="dash-cell {dc}">{d['day_chg']:+.2f}%</span>
  <span class="dash-cell {wc}">{d['perf_1w']:+.1f}%</span>
  <span class="dash-cell {mc}">{d['perf_1m']:+.1f}%</span>
  <span class="dash-cell {qc}">{d['perf_3m']:+.1f}%</span>
  <span class="dash-cell {adr_cls}">{adr_disp}</span>
  <span class="dash-cell {hc}">{d['pct_52h']:+.1f}%</span>
</div>""", unsafe_allow_html=True)
    ...
    # SECTION 6 — VIX + NOTES
    # ─────────────────────────────────────────────────────────────────────────
    st.markdown('<div class="dash-section-title">⑥ Market Environment</div>',
                unsafe_allow_html=True)

    vix_data = fetch_ticker_metrics("^VIX")
    vix_val  = vix_data["last"] if vix_data else None

    env_c1, env_c2 = st.columns([1, 3])
    with env_c1:
        if vix_val:
            vix_cls = "neg" if vix_val >= 25 else ("warn" if vix_val >= 18 else "pos")
            vix_lbl = "EXTREME FEAR" if vix_val >= 35 else "FEAR" if vix_val >= 25 else "ELEVATED" if vix_val >= 18 else "CALM"
            st.markdown(f"""
<div class="metric-cell">
  <span class="mc-label">VIX (Fear Index)</span>
  <span class="mc-val {vix_cls}">{vix_val:.1f}</span>
  <span class="mc-sub">{vix_lbl} · Jeff: ADR% > 5% required for setups</span>
</div>""", unsafe_allow_html=True)

    with env_c2:
        st.markdown("""
<div class="metric-cell" style="height:100%">
  <span class="mc-label">Jeff Sun's Daily Checklist (manual)</span>
  <div style="font-family:'DM Sans',sans-serif;font-size:0.72rem;color:var(--muted);margin-top:0.3rem;line-height:1.7">
    □ &nbsp;MMTW (% above 20-MA) &gt;50%? → Bullish bias&nbsp;&nbsp;&nbsp;
    □ &nbsp;NH/NL positive? → Breadth healthy<br>
    □ &nbsp;RSP vs SPY, QQQE vs QQQ diverging? → Equal-weight strength = broader participation<br>
    □ &nbsp;Leading sector in top 1–2 spots sustained? → Theme intact<br>
    □ &nbsp;Small cap (IWM) leading large cap (SPY)? → Risk-on<br>
    □ &nbsp;VIX below 20? → Calm enough to trade momentum
  </div>
</div>""", unsafe_allow_html=True)
